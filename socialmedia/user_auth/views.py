...
from .models import User,ProfileModel,Post,LikePost,Followers
...
def signup(request):
    try:
        if request.method == 'POST':
            ...
            my_profile = ProfileModel.objects.create(user=user_model,id_user=user_model.id)
            my_profile.save()
...
def profile(request,id_user):
    ...
    print(user_object)
    profile = ProfileModel.objects.get(user=request.user)
    user_profile = ProfileModel.objects.get(user=user_object)
...
def search_results(request):
    ...
    users = ProfileModel.objects.filter(user__username__icontains=query)
